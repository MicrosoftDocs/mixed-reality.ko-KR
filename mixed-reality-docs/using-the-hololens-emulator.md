...
HoloLens 에뮬레이터를 사용하면 실제 HoloLens 없이도 PC에서 홀로그램 앱을 테스트할 수 있으며, HoloLens 개발 도구 집합이 함께 제공됩니다. 에뮬레이터는 Hyper-V 가상 머신을 사용합니다. 일반적으로 HoloLens의 센서가 판독하는 휴먼 및 환경적 입력은 키보드, 마우스 또는 Xbox 컨트롤러를 사용하여 시뮬레이션됩니다. 에뮬레이터에서 실행하기 위해 앱을 수정할 필요가 없으며, 앱은 실제 HoloLens에서 실행되지 않는다는 걸 알지 못합니다.

데스크톱 PC용 Windows Mixed Reality 몰입형(VR) 헤드셋 앱이나 게임을 개발하려는 경우 데스크톱 헤드셋을 대신 시뮬레이션할 수 있는 [Windows Mixed Reality 시뮬레이터](using-the-windows-mixed-reality-simulator.md)를 확인하세요.


## <a name="installing-the-hololens-emulator"></a>HoloLens 에뮬레이터 설치
HoloLens 에뮬레이터 및 홀로그램 프로젝트 템플릿을 다운로드합니다.

버전: 
* [HoloLens 2 에뮬레이터 및 홀로그램 프로젝트 템플릿](https://go.microsoft.com/fwlink/?linkid=2087187).
* [HoloLens 에뮬레이터(1세대) 및 홀로그램 프로젝트 템플릿](https://go.microsoft.com/fwlink/?linkid=2065980).

HoloLens 에뮬레이터의 이전 빌드는 [HoloLens 에뮬레이터 아카이브](hololens-emulator-archive.md) 페이지에서 찾을 수 있습니다.
...
* RAM 8GB 이상
* BIOS에서 다음 기능이 [지원되어야 하며, 사용하도록 설정되어야 합니다](http://blogs.technet.com/b/iftekhar/archive/2010/08/09/enable-hardware-settings-in-bios-to-run-hyper-v.aspx).
   * 하드웨어 지원 가상화
...
시스템이 위의 요구 사항을 충족하는 경우 제어판 -> 프로그램 -> 프로그램 및 기능 -> Windows 기능 켜기 또는 끄기에서 -> 에뮬레이터가 성공적으로 설치되도록 “Hyper-V”가 선택되어 있는지 확인하여 **“Hyper-V” 기능이 사용하도록 설정되어 있는지 확인합니다**.

## <a name="deploying-apps-to-the-hololens-emulator"></a>HoloLens 에뮬레이터에 앱 배포

1. Visual Studio에서 앱 솔루션을 로드합니다.
    >[!NOTE]
    >Unity를 사용하면 Unity에서 프로젝트를 빌드한 다음, 빌드된 솔루션을 Visual Studio로 정상적으로 로드합니다.
...
4. **디버그 > 디버깅 시작**으로 이동하거나, **F5** 키를 눌러 에뮬레이터를 시작하고 디버깅을 위해 앱을 배포합니다.

에뮬레이터는 처음 시작할 때 부팅하는 데 1분 이상 걸릴 수 있습니다. 실행 중인 에뮬레이터에 앱을 신속하게 배포할 수 있도록 디버깅 세션 중에 에뮬레이터를 열어 두는 것이 좋습니다.

## <a name="basic-emulator-input"></a>기본 에뮬레이터 입력

에뮬레이터를 제어하는 것은 많은 일반적인 3D 비디오 게임과 매우 비슷합니다. 키보드, 마우스 또는 Xbox 컨트롤러를 사용하여 사용할 수 있는 입력 옵션이 있습니다. HoloLens를 착용한 시뮬레이션된 사용자의 작업을 지시하여 에뮬레이터를 제어합니다. 그러한 동작은 시뮬레이션된 사용자를 움직이며 에뮬레이터에서 실행 중인 앱은 실제 디바이스에서와 같이 응답합니다.
...
* **손 이동 및 방향**(HoloLens 2 에뮬레이터만 해당) - Alt 키를 누른 채 마우스를 위쪽/아래쪽/왼쪽/오른쪽으로 끌어 손을 이동하거나, 화살표 키 및 Q/E를 사용하여 손을 회전하고 기울입니다.  Xbox 컨트롤러의 경우 왼쪽 또는 오른쪽 범퍼를 누른 채로 왼쪽 엄지스틱을 사용하여 손을 왼쪽/오른쪽/앞으로/뒤로 움직입니다. 왼쪽 엄지스틱을 사용하여 회전시키고 Dpad에서 위쪽/아래쪽으로 움직여 손을 들거나 내립니다.
...
시뮬레이션 제어판을 사용하면 시뮬레이션된 휴먼 및 시뮬레이션된 입력 디바이스의 현재 위치와 방향을 볼 수 있습니다.  또한 한 손이나 두 손을 표시하거나 숨기는 것과 같은 시뮬레이션된 입력과 PC의 키보드, 마우스, 게임 패드와 같은 시뮬레이션된 입력을 제어하는 데 사용되는 디바이스 모두를 구성할 수 있습니다.
...
* 키보드, 마우스 또는 게임 패드 입력에 따라 시뮬레이션된 휴먼 또는 시뮬레이션된 입력 디바이스가 이동하거나 회전하는 속도를 조정하려면 “입력 설정” 옆의 기어 아이콘을 클릭하고 슬라이더를 조정합니다.
...
에뮬레이터를 시작하면 HoloLens OS를 표시하는 창이 표시됩니다.
...
에뮬레이터는 시뮬레이트된 “공간”에서 공간적 매핑 메시의 형태로 세계 입력을 시뮬레이션합니다. 이 탭에서는 기본 공간을 대신하여 로드할 공간을 선택할 수 있습니다.
...
시뮬레이션된 공간은 여러 환경에서 앱을 테스트하는 데 유용합니다. 에뮬레이터와 함께 여러 공간이 제공되고 에뮬레이션을 설치한 후 %ProgramFiles(x86)%\Windows Kits\10\Microsoft XDE\\(version)\Plugins\Rooms에서 찾을 수 있습니다. 이러한 모든 공간은 HoloLens를 사용하여 실제 환경에서 캡처되었습니다.
...
에뮬레이터에서는 렌더링하는 홀로그램만 표시되고, 홀로그램 뒤 시뮬레이션된 공간은 표시되지 않습니다. 이것은 두 가지가 함께 혼합되어 보이는 실제 HoloLens와는 대조적입니다. HoloLens 에뮬레이터에서 시뮬레이트된 공간을 보려면 장면에서 공간적 매핑 메시를 렌더링하도록 앱을 업데이트해야 합니다.

## <a name="troubleshooting"></a>문제 해결

*“Visual Studio 2015 업데이트 1 및 UWP 도구 버전 1.2”* 가 필요한 에뮬레이터를 설치하는 동안 오류가 표시될 수 있습니다. 이 오류의 가능한 세 가지 원인은 다음과 같습니다.
...
* Visual Studio 프로젝트 구성이 x86(HoloLens 1세대) 또는 x86 또는 x64(HoloLens 2 에뮬레이터)로 설정되어 있습니다.
...
에뮬레이터에서 시작 시 “잘못된 바이트 인코딩” 오류 대화 상자가 표시되는 경우:
* %localappdata%\Microsoft\XDE\HCS의 모든 파일을 삭제하고 다시 시도합니다.

Visual Studio의 디버그 대상 목록이 비어 있고(예를 들어, “시작”이 유일한 옵션) 위의 모든 문제 해결 단계를 수행한 경우:
