...
HoloLens 에뮬레이터를 사용하면 실제 HoloLens 없이 PC에서 홀로그램 애플리케이션을 테스트할 수 있습니다. 또한 HoloLens 개발 도구 세트가 포함됩니다. 에뮬레이터는 Hyper-V 가상 머신을 사용합니다. 일반적으로 HoloLens 센서가 판독하는 휴먼 및 환경적 입력은 키보드, 마우스 또는 Xbox 컨트롤러를 사용하여 시뮬레이션됩니다. 에뮬레이터에서 실행하기 위해 애플리케이션을 수정할 필요가 없으며, 애플리케이션은 실제 HoloLens에서 실행되지 않는다는 걸 알지 못합니다.
...
버전: 
* [HoloLens 2 에뮬레이터 및 홀로그램 프로젝트 템플릿](https://go.microsoft.com/fwlink/?linkid=2098508).
...
에뮬레이터는 처음 시작할 때 부팅하는 데 1분 이상 걸릴 수 있습니다. 에뮬레이터에 애플리케이션을 신속하게 배포할 수 있도록 디버깅 세션 중에 에뮬레이터를 열어 두는 것이 좋습니다.
