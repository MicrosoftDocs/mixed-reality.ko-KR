...
음성 입력 처리기 (스크립트) 구성 요소에서 작은 **+** 단추를 클릭 하 여 키워드를 추가한 다음, **키워드** 드롭다운에서 이전에 만든 **음악 재생** 키워드를 선택 합니다.
...
계층 구조 창에서 **4 개의 큐브** 를 창 **콘텐츠** 의 자식 개체로 만들고 해당 변환 **배율을** X = 0.15, Y = 0.15, Z = 0.15로 설정 합니다.
...
계층 구조 창에서 **치즈** 개체를 선택 하 고, **선택 된 () 이벤트에** 새 이벤트를 만들고, 이벤트를 받도록 **치즈** 개체를 구성 하 고, 트리거할 액션으로 RotateTarget를 정의 **합니다** .
