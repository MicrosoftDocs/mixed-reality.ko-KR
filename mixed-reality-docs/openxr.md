...
* **공급 업체 간 `EXT` 확장:** 여러 회사가 정의 하 고 구현 하는 교차 공급 업체 확장  관심이 있는 회사 그룹은 언제 든 지 내선 내선 번호를 도입할 수 있습니다.
* **공식 `KHR` 확장:** 공식 Khronos 확장은 핵심 사양 릴리스의 일부로 비준.  KHR 확장은 코어 사양 자체와 동일한 라이선스로 적용 됩니다.

2020 년 7 월 까지는 Windows Mixed Reality OpenXR 런타임이 HoloLens 2 기능의 전체 집합을 OpenXR 응용 프로그램으로 가져오는 `MSFT` 및 `EXT` 확장 집합을 지원 합니다.
...
| Direct3D 11 통합 | **정식 `KHR` 확장 릴리스:**<br /><code><a href="https://www.khronos.org/registry/OpenXR/specs/1.0/html/xrspec.html#XR_KHR_D3D11_enable" target="_blank">XR_KHR_D3D11_enable</a></code><br /> |
| Direct3D 12 통합 | **공식 `KHR` 확장 정의:** *(아직 지원 되지 않음)*<br /><code><a href="https://www.khronos.org/registry/OpenXR/specs/1.0/html/xrspec.html#XR_KHR_D3D12_enable" target="_blank">XR_KHR_D3D12_enable</a></code><br /><p>**Preview 지원**: 4 월 2020 *(계획 됨)*</p><p>**전체 지원**: 2020 년 5 월 *(계획 됨)*</p> |
...
이러한 확장 중 일부는 공급 업체별 `MSFT` 확장으로 시작 될 수 있지만 이러한 기능 영역에 대 한 공급 업체 `EXT` 또는 `KHR` 확장을 설계 하기 위해 Microsoft와 기타 OpenXR runtime 공급 업체를 함께 사용 하 고 있습니다.  이렇게 하면 핵심 사양과 마찬가지로 해당 기능에 대해 작성 하는 코드를 런타임 공급 업체 간에 이식할 수 있습니다.
