...
> 참고: Azure 공간 앵커를 가져오는 방법에 대 한 구체적인 지침은 5 단계를 참조 하 고, MR 기본 모듈 자산 팩의 특정 지침에 대해서는 6 단계를 참조 하 고, 혼합 현실 도구 키트 (MRKT)에 대 한 특정 지침은 3 ~ 4 단계를 참조 하세요.
...
> 참고: 팝업이 표시 되는 경우 혼합 현실 도구 키트에 대 한 파일을 선택 해야 합니다. 확인을 클릭 하면 4 단계로 이동 합니다.
...
> 팁: 이러한 모든 자산을 동일한 폴더에 저장 하 여 쉽게 찾아서 액세스할 수 있도록 합니다. 모든 것을 잘 구성 된 상태로 유지 합니다.
...
참고: ButtonParent를 장면에 추가 하면 TMP 자산을 가져올지 묻는 팝업이 표시 됩니다. "TMP Essentials"만 가져옵니다. 그러면 장면에 많은 글꼴 텍스트가 표시 되 면 ButtonParent 개체를 삭제 하 고 ASAmoduleAssets 폴더에서 다시 추가 합니다.
