...
1. 계층을 마우스 오른쪽 단추로 클릭 하 고 빈 만들기를 선택 하 여 빈 게임 개체를 만듭니다. 이름을 3DObjectCollection로 바꿉니다. 여기에 모든 3D 개체를 배치합니다. 컬렉션의 위치를 x = 0, y = 0 및 z = 0에 설정해야 합니다.

![Lesson4 Chapter1 Step1im](images/Lesson4_Chapter1_step1im.PNG)

2. 동일한 지침을 사용 하 여 [1 단원](mrlearning-base-ch1.md)에 설명 된 사용자 지정 패키지를 가져오는 basemodule 자산을 가져옵니다. BaseModule 자산에는이 자습서 전체에서 사용 되는 3D 모듈 및 기타 유용한 스크립트가 포함 되어 있습니다. BaseModule Unity 패키지는 다음 위치에서 찾을 수 있습니다. <https://github.com/Microsoft/MixedRealityLearning/releases/tag/V1.1>
...
5. 이제 장면에 더 많은 3D 개체를 추가 합니다. 다음은이 예제에서 추가할 개체의 목록입니다. 개체를 추가 하면 여러 크기의 장면에 표시 되는 것을 알 수 있습니다. 검사기 패널의 변형 설정에서 각 3D 모델의 배율을 조정 합니다. 이 예제에서 권장되는 조정은 아래 개체를 사용하여 나열됩니다. 프로젝트 패널의 검색 상자에서 이러한 단어를 검색 하 고 3D 개체 prefab을 이전 단계와 유사한 3DObjectCollection 개체로 끌어 옵니다. 이러한 prefabs의 컬렉션은의 > 자산에서 찾을 수 있습니다 > 기본 모듈 Prefabs
...
>참고: 일부 개체가 위의 이미지에 있는 개체와 같이 가운데에 있는 것을 확인할 수 있습니다. 이는 프리팹 또는 개체에 정렬되지 않은 자식 개체가 있기 때문입니다. 자유롭게 개체 위치 또는 자식 개체 위치를 조정하여 잘 정렬된 그리드를 만들 수 있습니다.
...
>참고: 큐브의 색을 선택할 수 있습니다. 이 예에서는 glowingcyan, glowingcyan 및 green이 사용 됩니다. 자유롭게 다른 색을 시도해 보세요. 큐브에 색을 추가 하려면 변경할 큐브를 클릭 한 다음 큐브의 검사기 패널에서 재질을 메시 렌더러의 재질 필드로 끕니다. 
...
>참고: 조작 처리기 구성 요소 및 연결 된 설정에 대 한 전체 설명서는 [Mrtk 설명서](https://microsoft.github.io/MixedRealityToolkit-Unity/Documentation/README_ManipulationHandler.html)를 참조 하세요.
...
> 참고:이 설정은 throw 할 수 있는 공을 만드는 등의 시나리오에 유용 합니다. 적절 한 속도 및 각도의 속도를 유지 하 여 해골을 릴리스한 후에는 릴리스되는 속도로 계속 이동 합니다. 물리적 공이 동작 하는 방식과 유사 합니다.
...
>참고: 개체에 경계 상자를 추가 하려면 먼저이 단원의 앞에서 설명한 것 처럼 개체 (예: 상자 collider)에 collider가 있어야 합니다. Colliders 개체를 선택 하 고 개체의 검사자 패널에서 구성 요소 추가 > 상자 Collider를 선택 하 여 추가할 수 있습니다.
>
...
> 참고: 상자 크기를 너무 크거나 너무 작게 지정 해야 합니다. 둘러싸고 있는 개체(이 예제에서는 지구 코어)와 대략 동일한 크기여야 합니다. Collider 상자에서 Collider 편집 옵션을 선택 하 여 필요에 따라 상자를 조정 합니다. X, y 및 z 값을 변경 하거나 편집기 장면 창에서 경계 상자 처리기를 끌 수 있습니다. 
...
8. 경계 상자가 오른쪽 개체를 대상으로 하는지 확인합니다. 경계 상자 구성 요소에는 대상 개체 및 경계 재정의 스크립트가 있습니다. 경계 상자를 포함 하는 개체를 두 슬롯 모두로 끕니다. 이 예제에서는 아래 이미지에 표시 된 것 처럼 node_id30 개체를 이러한 두 슬롯으로 끌어 옵니다.
...
>참고: Octa 개체에는 collider box가 있어야 합니다.
...
5. 아래 이미지에서 녹색 사각형 위의 함수를 표시 하지 않는 드롭다운에서 PlayOneShot를 > 선택 합니다. 아래 개념을 사용하여 오디오 클립을 이 필드에 추가하겠습니다.
   ...
   - MRTK는 오디오 클립의 작은 목록을 제공합니다. 프로젝트 패널에서 자유롭게 살펴보세요. MixedRealityToolkit 폴더 아래와 표준 자산 폴더 아래에서 찾을 수 있습니다. 여기에 모든 오디오 클립이 있는 오디오 폴더가 표시 됩니다.
   - 이 예에서는 MRTK_Gem 오디오 클립을 사용 합니다. 
   - 오디오 클립을 추가 하려면 검사기 패널에서 프로젝트 패널의 클립을 PlayOneShot (위 예제의 녹색 상자로 표시)로 끌어 놓기만 하면 됩니다.

   이제 사용자가 Octa 개체에 도달 하 여 닿을 때 오디오 트랙 MRTK_Gem가 재생 됩니다. 또한 직접 상호 작용 터치 스크립트는 개체의 색을 조정 합니다. 
