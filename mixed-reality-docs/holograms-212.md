...
* 응용 프로그램을 HoloLens에 배포한 후에는 마우스를 [탭](gaze-and-commit.md#composite-gestures) 제스처를 사용 하 여 맞춤 상자를 해제 합니다.
...
그럼 시작해 보겠습니다. **MicrophoneManager.cs**에서 3. a에 대 한 모든 코딩 연습을 완료 하거나 아래에 있는 완성 된 코드를 복사 하 여 붙여넣습니다.
...
* 3 가지 "color + shape" 규칙의 조합을 수신 하는 루트 규칙 colorChooser입니다. 도형은 임의의 순서로 정렬 하 고 1부터 모두 세까지 지정할 수 있습니다. 이 규칙은 초기 &lt;문법&gt; 태그에서 파일의 맨 위에 있는 루트 규칙으로 지정 되므로에서 수신 하는 유일한 규칙입니다.
