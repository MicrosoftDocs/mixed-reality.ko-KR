...
- [<span data-ttu-id="f5aa5-147">Visual Studio 2017</span><span class="sxs-lookup"><span data-stu-id="f5aa5-147">Visual Studio 2017</span></span>](https://docs.microsoft.com/windows/mixed-reality/install-the-tools#installation-checklist-for-hololens)
...
<span data-ttu-id="f5aa5-160">**Azure Custom Vision Service**을 사용 하려면 응용 프로그램에서 사용할 수 있도록 인스턴스를 구성 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-160">To use the **Azure Custom Vision Service**, you will need to configure an instance of it to be made available to your application.</span></span>
...
14. <span data-ttu-id="f5aa5-214">**예측 URL**을 클릭 하면 *메모장*을 열고, 코드에서 나중에 필요할 때 검색할 수 있도록 **url** ( **예측 엔드포인트**라고도 함) 및 **서비스 예측 키**를 복사 하 여 붙여넣습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-214">Once you have clicked on **Prediction URL**, open *Notepad*, and copy and paste the **URL** (also called your **Prediction-Endpoint**) and the **Service Prediction-Key**, so that you can retrieve it when you need it later in the code.</span></span>
...
5.  <span data-ttu-id="f5aa5-227">동일한 **빌드 설정** 창에서 다음이 설정 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-227">In the same **Build Settings** window, ensure the following are set:</span></span>
...
7. <span data-ttu-id="f5aa5-235">이 패널에서 몇 가지 설정을 확인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-235">In this panel, a few settings need to be verified:</span></span>
    ...
    2.  <span data-ttu-id="f5aa5-240">**게시 설정** 탭의 **기능**아래에서 다음을 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-240">Within the **Publishing Settings** tab, under **Capabilities**, check:</span></span>
        ...
        3. <span data-ttu-id="f5aa5-243">**SpatialPerception**</span><span class="sxs-lookup"><span data-stu-id="f5aa5-243">**SpatialPerception**</span></span>
            ...
            <span data-ttu-id="f5aa5-244">![](images/AzureLabs-Lab310-27.png) ![](images/AzureLabs-Lab310-28.png)</span><span class="sxs-lookup"><span data-stu-id="f5aa5-244">![](images/AzureLabs-Lab310-27.png) ![](images/AzureLabs-Lab310-28.png)</span></span>
...
> <span data-ttu-id="f5aa5-257">잠깐만 전체 장면을 포함 하 여 Unity에서 지원 되는 모든 개체를 **unitypackage** 파일로 패키지 하 고 다른 프로젝트에서 내보내거나 가져올 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-257">[TIP] Any objects supported by Unity, including entire scenes, can be packaged into a **.unitypackage** file, and exported / imported in other projects.</span></span> <span data-ttu-id="f5aa5-258">서로 다른 **Unity 프로젝트**간에 자산을 이동 하는 가장 안전 하 고 가장 효율적인 방법입니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-258">It is the safest, and most efficient, way to move assets between different **Unity projects**.</span></span>
...
3.  <span data-ttu-id="f5aa5-264">가져오기가 완료 되 면 패키지의 폴더가 **자산** 폴더에 추가 된 것을 알 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-264">Once it has finished importing, you will notice that folders from the package have now been added to your **Assets** folder.</span></span> <span data-ttu-id="f5aa5-265">이러한 종류의 폴더 구조는 Unity 프로젝트에 일반적입니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-265">This kind of folder structure is typical for a Unity project.</span></span>
    ...
    3.  <span data-ttu-id="f5aa5-269">**Prefabs** 폴더에는 장면에 포함 된 Prefabs이 포함 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-269">The **Prefabs** folder contains the prefabs contained in the scene.</span></span> <span data-ttu-id="f5aa5-270">이러한 작업은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-270">Those are:</span></span>
        ...
        1.  <span data-ttu-id="f5aa5-271">응용 프로그램에서 사용 되는 **GazeCursor**입니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-271">The **GazeCursor**, the cursor used in the application.</span></span> <span data-ttu-id="f5aa5-272">는 SpatialMapping prefab와 함께 작동 하 여 물리적 개체의 맨 위에 장면에 배치할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-272">Will work together with the SpatialMapping prefab to be able to be placed in the scene on top of physical objects.</span></span>
...
<span data-ttu-id="f5aa5-285">이 클래스는 다음을 담당 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-285">This class is responsible for:</span></span>
...
9. <span data-ttu-id="f5aa5-305">**Start ()** 및 **Update ()** 메서드를 사용 하지 않으므로 삭제 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-305">Delete the **Start()** and **Update()** methods, as they will not be used.</span></span> 
...
7.  <span data-ttu-id="f5aa5-332">**Unity**로 반환 하기 전에 **Visual Studio**에서 변경 내용을 저장 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-332">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>
...
<span data-ttu-id="f5aa5-346">이 클래스는 다음을 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-346">This class will:</span></span>
...
7.  <span data-ttu-id="f5aa5-357">**PlaceAnalysisLabel ()** 메서드를 추가 합니다 .이 메서드는 장면 (이 시점에서 사용자에 게 보이지 않음)의 레이블을 *인스턴스화합니다* .</span><span class="sxs-lookup"><span data-stu-id="f5aa5-357">Add the **PlaceAnalysisLabel()** method, which will *Instantiate* the label in the scene (which at this point is invisible to the user).</span></span> <span data-ttu-id="f5aa5-358">또한 이미지가 배치 되는 4 개 (보이지 않음)를 배치 하 고 실제 세계와 겹칩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-358">It also places the quad (also invisible) where the image is placed, and overlaps with the real world.</span></span> <span data-ttu-id="f5aa5-359">분석 후 서비스에서 검색 된 상자 좌표가이 쿼드로 다시 추적 되어 실제 세계에 있는 개체의 대략적인 위치를 결정 하기 때문에이는 중요 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-359">This is important because the box coordinates retrieved from the Service after analysis are traced back into this quad to determined the approximate location of the object in the real world.</span></span> 
...
8.  <span data-ttu-id="f5aa5-360">**FinaliseLabel ()** 메서드를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-360">Add the **FinaliseLabel()** method.</span></span> <span data-ttu-id="f5aa5-361">이 담당 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-361">It is responsible for:</span></span>

    *   <span data-ttu-id="f5aa5-362">신뢰 수준이 가장 높은 예측의 *태그로* *레이블* 텍스트를 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-362">Setting the *Label* text with the *Tag* of the Prediction with the highest confidence.</span></span>
    *   <span data-ttu-id="f5aa5-363">이전에 배치 하 고 장면에 레이블을 배치 하 여 쿼드 개체의 *경계 상자* 계산 호출</span><span class="sxs-lookup"><span data-stu-id="f5aa5-363">Calling the calculation of the *Bounding Box* on the quad object, positioned previously, and placing the label in the scene.</span></span>
    *   <span data-ttu-id="f5aa5-364">실제 세계의 개체와 충돌 하는 *경계 상자*를 향해 Raycast를 사용 하 여 레이블 깊이를 조정 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-364">Adjusting the label depth by using a Raycast towards the *Bounding Box*, which should collide against the object in the real world.</span></span>
    * <span data-ttu-id="f5aa5-365">사용자가 다른 이미지를 캡처할 수 있도록 캡처 프로세스를 다시 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-365">Resetting the capture process to allow the user to capture another image.</span></span>
...
9.  <span data-ttu-id="f5aa5-366">**CalculateBoundingBoxPosition ()** 메서드를 추가 합니다 .이 메서드는 서비스에서 검색 된 *경계 상자* 좌표를 변환 하 고 쿼드에 비례하여 다시 만드는 데 필요한 계산을 여러 개 호스팅합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-366">Add the **CalculateBoundingBoxPosition()** method, which hosts a number of calculations necessary to translate the *Bounding Box* coordinates retrieved from the Service and recreate them proportionally on the quad.</span></span>
...
10. <span data-ttu-id="f5aa5-367">**Unity**로 반환 하기 전에 **Visual Studio**에서 변경 내용을 저장 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-367">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>

    > [!IMPORTANT]
    > <span data-ttu-id="f5aa5-368">계속 하기 전에 **CustomVisionAnalyser** 클래스를 열고 **AnalyseLastImageCaptured ()** 메서드 내에서 다음 줄의 *주석 처리를 제거* 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-368">Before you continue, open the **CustomVisionAnalyser** class, and within the **AnalyseLastImageCaptured()** method, *uncomment* the following lines:</span></span>
    >
    >   ```csharp
    >   // Create a texture. Texture size does not matter, since 
    >   // LoadImage will replace with the incoming image size.
...
> [!NOTE]
> <span data-ttu-id="f5aa5-369">**ImageCapture** 클래스 ' 찾을 수 없음 ' 메시지에 대해 걱정 하지 마세요. 다음 장에서 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-369">Do not worry about the **ImageCapture** class 'could not be found' message, you will create it in the next chapter.</span></span>


## <a name="chapter-10---create-the-imagecapture-class"></a><span data-ttu-id="f5aa5-370">10 장-ImageCapture 클래스 만들기</span><span class="sxs-lookup"><span data-stu-id="f5aa5-370">Chapter 10 - Create the ImageCapture class</span></span>

<span data-ttu-id="f5aa5-371">만들려는 다음 클래스는 **ImageCapture** 클래스입니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-371">The next class you are going to create is the **ImageCapture** class.</span></span>

<span data-ttu-id="f5aa5-372">이 클래스는 다음을 담당 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-372">This class is responsible for:</span></span>

*   <span data-ttu-id="f5aa5-373">HoloLens 카메라를 사용 하 여 이미지를 캡처하고 *앱* 폴더에 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-373">Capturing an image using the HoloLens camera and storing it in the *App* folder.</span></span>
*   <span data-ttu-id="f5aa5-374">사용자 로부터 *탭* 제스처를 처리 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-374">Handling *Tap* gestures from the user.</span></span>

<span data-ttu-id="f5aa5-375">이 클래스를 만들려면:</span><span class="sxs-lookup"><span data-stu-id="f5aa5-375">To create this class:</span></span>

1.  <span data-ttu-id="f5aa5-376">이전에 만든 **스크립트** 폴더로 이동 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-376">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="f5aa5-377">폴더 내부를 마우스 오른쪽 단추로 클릭 한 다음,**C\# 스크립트** **만들기** > 를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-377">Right-click inside the folder, then click **Create** > **C\# Script**.</span></span> <span data-ttu-id="f5aa5-378">스크립트 이름을 **ImageCapture**로 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-378">Name the script **ImageCapture**.</span></span>

3.  <span data-ttu-id="f5aa5-379">새 **ImageCapture** 스크립트를 두 번 클릭 하 여 **Visual Studio** 에서 엽니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-379">Double-click on the new **ImageCapture** script to open it with **Visual Studio.**</span></span>

4.  <span data-ttu-id="f5aa5-380">파일의 맨 위에 있는 네임 스페이스를 다음으로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-380">Replace the namespaces at the top of the file with the following:</span></span>
...
5.  <span data-ttu-id="f5aa5-381">그런 다음 **ImageCapture** 클래스 내에서 **Start ()** 메서드 위의 다음 변수를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-381">Then add the following variables inside the **ImageCapture** class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="f5aa5-382">이제 **활성 ()** 및 **Start ()** 메서드에 대 한 코드를 추가 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-382">Code for **Awake()** and **Start()** methods now needs to be added:</span></span>
...
7.  <span data-ttu-id="f5aa5-383">탭 제스처가 발생할 때 호출 되는 처리기를 구현 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-383">Implement a handler that will be called when a Tap gesture occurs:</span></span>
    ...
    > [!IMPORTANT]
    > <span data-ttu-id="f5aa5-384">커서가 **녹색이**면 카메라에서 이미지를 사용할 수 있음을 의미 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-384">When the cursor is **green**, it means the camera is available to take the image.</span></span> <span data-ttu-id="f5aa5-385">커서가 **빨간색**이면 카메라가 사용 중임을 의미 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-385">When the cursor is **red**, it means the camera is busy.</span></span>

8.  <span data-ttu-id="f5aa5-386">응용 프로그램이 이미지 캡처 프로세스를 시작 하 고 이미지를 저장 하는 데 사용 하는 메서드를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-386">Add the method that the application uses to start the image capture process and store the image:</span></span>
...
9.  <span data-ttu-id="f5aa5-387">사진이 캡처되고 분석 준비가 될 때 호출 되는 처리기를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-387">Add the handlers that will be called when the photo has been captured and for when it is ready to be analyzed.</span></span> <span data-ttu-id="f5aa5-388">그런 다음 결과가 분석을 위해 **CustomVisionAnalyser** 에 전달 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-388">The result is then passed to the **CustomVisionAnalyser** for analysis.</span></span>
...
10. <span data-ttu-id="f5aa5-389">**Unity**로 반환 하기 전에 **Visual Studio**에서 변경 내용을 저장 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-389">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>

## <a name="chapter-11---setting-up-the-scripts-in-the-scene"></a><span data-ttu-id="f5aa5-390">11 장-장면에서 스크립트 설정</span><span class="sxs-lookup"><span data-stu-id="f5aa5-390">Chapter 11 - Setting up the scripts in the scene</span></span>

<span data-ttu-id="f5aa5-391">이 프로젝트에 필요한 모든 코드를 작성 했으므로 이제는 장면에서 스크립트를 설정 하 고 prefabs에서 제대로 동작 하도록 하는 시간입니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-391">Now that you have written all of the code necessary for this project, is time to set up the scripts in the scene, and on the prefabs, for them to behave correctly.</span></span>

1.  <span data-ttu-id="f5aa5-392">**Unity 편집기**내의 **계층 패널**에서 **주 카메라**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-392">Within the **Unity Editor**, in the **Hierarchy Panel**, select the **Main Camera**.</span></span>
2.  <span data-ttu-id="f5aa5-393">**검사기 패널**에서 **주 카메라** 를 선택 하 고 **구성 요소 추가**를 클릭 한 다음 **SceneOrganiser** 스크립트를 검색 하 고 두 번 클릭 하 여 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-393">In the **Inspector Panel**, with the **Main Camera** selected, click on **Add Component**, then search for **SceneOrganiser** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-38.png)

3.  <span data-ttu-id="f5aa5-394">에 표시 된 것 처럼 **프로젝트 패널**에서 **Prefabs 폴더**를 열고 Prefab **label** 을 *기본 카메라*에 방금 추가한 **SceneOrganiser** 스크립트의 *레이블* 빈 참조 대상 입력 영역으로 끌어 옵니다. 아래 이미지:</span><span class="sxs-lookup"><span data-stu-id="f5aa5-394">In the **Project Panel**, open the **Prefabs folder**, drag the **Label** prefab into the *Label* empty reference target input area, in the **SceneOrganiser** script that you have just added to the *Main Camera*, as shown in the image below:</span></span>

    ![](images/AzureLabs-Lab310-39.png)

4.  <span data-ttu-id="f5aa5-395">**계층 패널**에서 **기본 카메라**의 **GazeCursor** 자식을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-395">In the **Hierarchy Panel**, select the **GazeCursor** child of the **Main Camera**.</span></span>
5.  <span data-ttu-id="f5aa5-396">**검사기 패널**에서 **GazeCursor** 가 선택 된 상태에서 **구성 요소 추가**를 클릭 한 다음 **GazeCursor** 스크립트를 검색 하 고 두 번 클릭 하 여 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-396">In the **Inspector Panel**, with the **GazeCursor** selected, click on **Add Component**, then search for **GazeCursor** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-40.png)

6.  <span data-ttu-id="f5aa5-397">**계층 패널**에서 **기본 카메라**의 **SpatialMapping** 자식을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-397">Again, in the **Hierarchy Panel**, select the **SpatialMapping** child of the **Main Camera**.</span></span>
7.  <span data-ttu-id="f5aa5-398">**검사기 패널**에서 **SpatialMapping** 가 선택 된 상태에서 **구성 요소 추가**를 클릭 한 다음 **SpatialMapping** 스크립트를 검색 하 고 두 번 클릭 하 여 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-398">In the **Inspector Panel**, with the **SpatialMapping** selected, click on **Add Component**, then search for **SpatialMapping** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-41.png)

<span data-ttu-id="f5aa5-399">설정 하지 않은 나머지 스크립트는 런타임 중에 **SceneOrganiser** 스크립트의 코드에 의해 추가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-399">The remaining scripts thats you have not set will be added by the code in the **SceneOrganiser** script, during runtime.</span></span>

## <a name="chapter-12---before-building"></a><span data-ttu-id="f5aa5-400">12 장-빌드하기 전</span><span class="sxs-lookup"><span data-stu-id="f5aa5-400">Chapter 12 - Before building</span></span>

<span data-ttu-id="f5aa5-401">응용 프로그램에 대 한 철저 한 테스트를 수행 하려면 Microsoft HoloLens로 테스트용으로 로드 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-401">To perform a thorough test of your application you will need to sideload it onto your Microsoft HoloLens.</span></span>

<span data-ttu-id="f5aa5-402">이렇게 하려면 먼저 다음을 확인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-402">Before you do, ensure that:</span></span>

-  <span data-ttu-id="f5aa5-403">[3 장](#chapter-3---set-up-the-unity-project) 에서 설명한 모든 설정이 올바르게 설정 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-403">All the settings mentioned in the [Chapter 3](#chapter-3---set-up-the-unity-project) are set correctly.</span></span>
- <span data-ttu-id="f5aa5-404">**SceneOrganiser** 스크립트는 **주 카메라** 개체에 연결 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-404">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span>
- <span data-ttu-id="f5aa5-405">**GazeCursor** 스크립트는 **GazeCursor** 개체에 연결 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-405">The script **GazeCursor** is attached to the **GazeCursor** object.</span></span>
- <span data-ttu-id="f5aa5-406">**SpatialMapping** 스크립트는 **SpatialMapping** 개체에 연결 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-406">The script **SpatialMapping** is attached to the **SpatialMapping** object.</span></span>
- <span data-ttu-id="f5aa5-407">[5 장](#chapter-5---create-the-customvisionanalyser-class), 6 단계:</span><span class="sxs-lookup"><span data-stu-id="f5aa5-407">In [Chapter 5](#chapter-5---create-the-customvisionanalyser-class), Step 6:</span></span>

    - <span data-ttu-id="f5aa5-408">**PredictionKey** 변수에 **서비스 예측 키** 를 삽입 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-408">Make sure you insert your **Service Prediction Key** into the **predictionKey** variable.</span></span>
    - <span data-ttu-id="f5aa5-409">**예측 끝점** 을 **predictionEndpoint** 클래스에 삽입 했습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-409">You have inserted your **Prediction Endpoint** into the **predictionEndpoint** class.</span></span>

## <a name="chapter-13---build-the-uwp-solution-and-sideload-your-application"></a><span data-ttu-id="f5aa5-410">13 장-UWP 솔루션 빌드 및 응용 프로그램 테스트용으로 로드</span><span class="sxs-lookup"><span data-stu-id="f5aa5-410">Chapter 13 - Build the UWP solution and sideload your application</span></span>

<span data-ttu-id="f5aa5-411">이제 Microsoft HoloLens에 배포할 수 있는 UWP 솔루션으로 응용 프로그램을 빌드할 준비가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-411">You are now ready to build you application as a UWP Solution that you will be able to deploy on to the Microsoft HoloLens.</span></span> <span data-ttu-id="f5aa5-412">빌드 프로세스를 시작 하려면 다음을 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-412">To begin the build process:</span></span>

1.  <span data-ttu-id="f5aa5-413">**파일 > 빌드 설정**으로 이동 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-413">Go to **File > Build Settings**.</span></span>

2.  <span data-ttu-id="f5aa5-414">Tick **Unity C\# 프로젝트**.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-414">Tick **Unity C\# Projects**.</span></span>

3.  <span data-ttu-id="f5aa5-415">열려 있는 **장면 추가**를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-415">Click on **Add Open Scenes**.</span></span> <span data-ttu-id="f5aa5-416">이렇게 하면 현재 열려 있는 장면이 빌드에 추가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-416">This will add the currently open scene to the build.</span></span>

    ![](images/AzureLabs-Lab310-42.png)

4.  <span data-ttu-id="f5aa5-417">**빌드**를 클릭합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-417">Click **Build**.</span></span> <span data-ttu-id="f5aa5-418">Unity는 응용 프로그램을 빌드할 폴더를 만들고 선택 해야 하는 *파일 탐색기* 창을 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-418">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="f5aa5-419">이제 해당 폴더를 만들고 이름을 **App**으로 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-419">Create that folder now, and name it **App**.</span></span> <span data-ttu-id="f5aa5-420">그런 다음 **앱** 폴더를 선택 하 고 **폴더 선택**을 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-420">Then with the **App** folder selected, click **Select Folder**.</span></span>

5.  <span data-ttu-id="f5aa5-421">Unity는 **응용** 프로그램 폴더에 대 한 프로젝트 빌드를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-421">Unity will begin building your project to the **App** folder.</span></span>

6.  <span data-ttu-id="f5aa5-422">Unity가 빌드를 완료 하면 (시간이 걸릴 수 있음) 빌드 위치에서 **파일 탐색기** 창이 열립니다. (작업 표시줄은 항상 창 위에 표시 되는 것은 아니지만 새 창 추가를 알려 줍니다.)</span><span class="sxs-lookup"><span data-stu-id="f5aa5-422">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

7.  <span data-ttu-id="f5aa5-423">Microsoft HoloLens에 배포 하려면 해당 장치의 IP 주소가 필요 하며 (원격 배포의 경우) **개발자 모드가** 설정 되어 있는지도 확인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-423">To deploy on to Microsoft HoloLens, you will need the IP Address of that device (for Remote Deploy), and to ensure that it also has **Developer Mode** set.</span></span> <span data-ttu-id="f5aa5-424">가상 하드 디스크 파일에 대한 중요 정보를 제공하려면</span><span class="sxs-lookup"><span data-stu-id="f5aa5-424">To do this:</span></span>

    1.  <span data-ttu-id="f5aa5-425">HoloLens를 입고 하는 동안 **설정을**엽니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-425">Whilst wearing your HoloLens, open the **Settings**.</span></span>

    2.  <span data-ttu-id="f5aa5-426">**네트워크 & 인터넷** > **wi-fi** > **고급 옵션** 으로 이동 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-426">Go to **Network & Internet** > **Wi-Fi** > **Advanced Options**</span></span>

    3.  <span data-ttu-id="f5aa5-427">**IPv4** 주소를 적어둡니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-427">Note the **IPv4** address.</span></span>

    4.  <span data-ttu-id="f5aa5-428">그런 다음 **설정**으로 다시 이동한 다음**개발자** 를 위한 **& 보안** > 을 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-428">Next, navigate back to **Settings**, and then to **Update & Security** > **For Developers**</span></span>

    5.  <span data-ttu-id="f5aa5-429">에서 **개발자 모드** *를*설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-429">Set **Developer Mode** *On*.</span></span>

8.  <span data-ttu-id="f5aa5-430">새 Unity 빌드 ( **앱** 폴더)로 이동 하 여 **Visual Studio**에서 솔루션 파일을 엽니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-430">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio**.</span></span>

9.  <span data-ttu-id="f5aa5-431">솔루션 구성에서 **디버그**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-431">In the Solution Configuration select **Debug**.</span></span>

10. <span data-ttu-id="f5aa5-432">솔루션 플랫폼에서 **x86, 원격 컴퓨터**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-432">In the Solution Platform, select **x86, Remote Machine**.</span></span> <span data-ttu-id="f5aa5-433">원격 장치의 **IP 주소** (이 경우에는 기록한 Microsoft HoloLens)를 삽입 하 라는 메시지가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-433">You will be prompted to insert the **IP address** of a remote device (the Microsoft HoloLens, in this case, which you noted).</span></span>

    ![](images/AzureLabs-Lab310-43.png)

11. <span data-ttu-id="f5aa5-434">**빌드** 메뉴로 이동 하 여 **솔루션 배포** 를 클릭 하 여 응용 프로그램을 HoloLens로 테스트용으로 로드.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-434">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your HoloLens.</span></span>

12. <span data-ttu-id="f5aa5-435">이제 앱이 Microsoft HoloLens의 설치 된 앱 목록에 표시 되어 시작 될 준비가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-435">Your app should now appear in the list of installed apps on your Microsoft HoloLens, ready to be launched!</span></span>

### <a name="to-use-the-application"></a><span data-ttu-id="f5aa5-436">응용 프로그램을 사용 하려면:</span><span class="sxs-lookup"><span data-stu-id="f5aa5-436">To use the application:</span></span>

* <span data-ttu-id="f5aa5-437">**Azure Custom Vision Service 및 개체 검색**을 사용 하 여 학습 하 고 **탭 제스처**를 사용 하 여 개체를 살펴보세요.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-437">Look at an object, which you have trained with your **Azure Custom Vision Service, Object Detection**, and use the **Tap gesture**.</span></span>
* <span data-ttu-id="f5aa5-438">개체가 성공적으로 검색 되 면 세계 공백 *레이블 텍스트가* 태그 이름과 함께 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-438">If the object is successfully detected, a world-space *Label Text* will appear with the tag name.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="f5aa5-439">사진을 캡처하여 서비스로 보낼 때마다 서비스 페이지로 돌아가서 새로 캡처한 이미지를 사용 하 여 서비스를 다시 학습 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-439">Every time you capture a photo and send it to the Service, you can go back to the Service page and retrain the Service with the newly captured images.</span></span> <span data-ttu-id="f5aa5-440">처음에는 *경계 상자* 를 수정 하 여 보다 정확 하 고 서비스를 다시 학습 하는 것도 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-440">At the beginning, you will probably also have to correct the *Bounding Boxes* to be more accurate and retrain the Service.</span></span>

> [!NOTE]
> <span data-ttu-id="f5aa5-441">Microsoft HoloLens 센서 및/또는 Unity의 SpatialTrackingComponent가 실제 개체를 기준으로 적절 한 colliders를 배치 하지 못할 때 배치 된 레이블 텍스트가 개체 근처에 나타나지 않을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-441">The Label Text placed might not appear near the object when the Microsoft HoloLens sensors and/or the SpatialTrackingComponent in Unity fails to place the appropriate colliders, relative to the real world objects.</span></span> <span data-ttu-id="f5aa5-442">해당 하는 경우 다른 화면에서 응용 프로그램을 사용 하십시오.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-442">Try to use the application on a different surface if that is the case.</span></span>

## <a name="your-custom-vision-object-detection-application"></a><span data-ttu-id="f5aa5-443">Custom Vision, 개체 검색 응용 프로그램</span><span class="sxs-lookup"><span data-stu-id="f5aa5-443">Your Custom Vision, Object Detection application</span></span>

<span data-ttu-id="f5aa5-444">축 하 합니다. Azure Custom Vision, 개체 검색 API를 활용 하는 혼합 현실 앱을 빌드하여 이미지에서 개체를 인식 한 다음 3D 공간에서 해당 개체에 대 한 대략적인 위치를 제공할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-444">Congratulations, you built a mixed reality app that leverages the Azure Custom Vision, Object Detection API, which can recognize an object from an image, and then provide an approximate position for that object in 3D space.</span></span>

![](images/AzureLabs-Lab310-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="f5aa5-445">보너스 연습</span><span class="sxs-lookup"><span data-stu-id="f5aa5-445">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="f5aa5-446">연습 1</span><span class="sxs-lookup"><span data-stu-id="f5aa5-446">Exercise 1</span></span>

<span data-ttu-id="f5aa5-447">텍스트 레이블에를 추가 하 여 반투명 큐브를 사용 하 여 3D *경계 상자*에 실제 개체를 래핑합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-447">Adding to the Text Label, use a semi-transparent cube to wrap the real object in a 3D *Bounding Box*.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="f5aa5-448">연습 2</span><span class="sxs-lookup"><span data-stu-id="f5aa5-448">Exercise 2</span></span>

<span data-ttu-id="f5aa5-449">Custom Vision Service를 학습 하 여 더 많은 개체를 인식할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-449">Train your Custom Vision Service to recognize more objects.</span></span>

### <a name="exercise-3"></a><span data-ttu-id="f5aa5-450">연습 3</span><span class="sxs-lookup"><span data-stu-id="f5aa5-450">Exercise 3</span></span>

<span data-ttu-id="f5aa5-451">개체가 인식 될 때 소리를 재생 합니다.</span><span class="sxs-lookup"><span data-stu-id="f5aa5-451">Play a sound when an object is recognized.</span></span>

### <a name="exercise-4"></a><span data-ttu-id="f5aa5-452">연습 4</span><span class="sxs-lookup"><span data-stu-id="f5aa5-452">Exercise 4</span></span>

<span data-ttu-id="f5aa5-453">API를 사용 하 여 응용 프로그램이 분석 하는 것과 동일한 이미지를 사용 하 여 서비스를 다시 학습 하므로 서비스를 보다 정확 하 게 만들 수 있습니다 (예측 및 학습을 동시에 수행).</span><span class="sxs-lookup"><span data-stu-id="f5aa5-453">Use the API to re-train your Service with the same images your app is analyzing, so to make the Service more accurate (do both prediction and training simultaneously).</span></span>
